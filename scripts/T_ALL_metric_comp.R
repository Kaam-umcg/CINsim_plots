...
best_sim_dir <- file.path(
  Sys.getenv("SCRATCH"),
  "CINsim",
  "best_sims",
)

optimal_sims_surv <- readRDS(file.path(
  best_sim_dir,
  "surv_FC_mono_penalty.Rds"
))

optimal_sims_div <- readRDS(file.path(
  best_sim_dir,
...
for (condition in names(best_sims)){
  ...
  surviving_sims <- unlist(map(1:ITERATIONS, ~ CINsim::check_viability(sims, .x, 
  ...
  CnFS <- unlist(lapply(sims, get_final_CnFS))
...
#TODO
# MAKE SURE THE PLOT BELOW HERE IS GOOD
