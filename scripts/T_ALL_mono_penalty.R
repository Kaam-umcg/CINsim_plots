# hard imports - this does raise some conflicts!
library(CINsim)
library(tidyr)
library(reshape2)
library(ggplot2)
...
p1 <- ggplot(melt_Mps1, aes(x = Chromosome, y = Fraction, 
...
ggplot2::ggsave(
  file = file.path(
    plot_dir,
    "selection_frequencies_Mps1.pdf"
  ),
  plot = p1)
...
survival_FCs <- make_cinsim_coeffcients(selection_metric = Mps1,
                                        euploid_copy = 2,
                                        min_survival_euploid = 0.4, #survival_FC = 2.5
                                        max_survival_euploid =  0.9, #survival_FC = 1.111.. 
                                        max_survival = 1,
                                        interval = 0.5 / 24,         # takes 25 samples in the interval 0.5
                                        probability_types = c("pSurvival"))
survival_FC_vals = (1 / seq(0.4, 0.9, 0.5 / 24))
...
p2 <- ggplot(sim_df, aes(x = survival_FCs, y = pMisseg)) +
...
p4 <- cnvHeatmap(selected_sim, subset_size = 1000)
...
p5 <- ggplot(bar_plot_frame, aes(x = Score, y = Value, fill = Group, group = Group)) +
