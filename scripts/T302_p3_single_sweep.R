...
plot_dir <- file.path(
  TMP_DIR,
  "plots",
  paste0("single_sweep_T302_p3_", as.character(wanted_survival_FC))
)
...
for (i in 1:nrow(sim_df)){
  ...
  surviving_sims <- unlist(map(1:ITERATIONS, ~ CINsim::check_viability(sim_list, .x, threshold_value = MAX_CELLS, max_g = GENERATIONS)))
...
ggplot2::ggsave(
  ...
  file = file.path(
    plot_dir,
    paste0(
      "CnFS_viability_heatmap_T302_p3_sweep_",
      as.character(wanted_survival_FC),
      ".pdf")
  )
)
...
ggplot2::ggsave(
  ...
  file = file.path(
    plot_dir,
    paste0(
      "karyotype_landscape_T302_p3_sweep_",
      as.character(wanted_survival_FC),
      ".pdf")
    )
)
...
ggplot2::ggsave(
  ...
  file = file.path(
    plot_dir,
    paste0(
      "sample_karyotypes_T302_p3_single_sweep_",
      as.character(wanted_survival_FC),
      ".pdf")
    )
)
