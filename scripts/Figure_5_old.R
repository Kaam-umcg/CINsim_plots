...
for (sample_name in ORGANOID_NAMES){
    ...
    best_diploid_sim_fn <- paste0("diploid", "_misseg_", round(best_params_diploid$p_misseg, 7),
    ...
    diploid_sims <- readRDS(file.path(DIPLOID_RESULTS, best_diploid_sim_fn))
    ...
    print(best_params_diploid)
    print(best_params_WGD)

    best_WGD_sim_fn <- paste0("WGD", "_misseg_", round(best_params_WGD$p_misseg, 7),
                   "_div_FC_", best_params_WGD$division_FC,
                   "_surv_FC_", best_params_WGD$survival_FC, ".Rds")

    # if we load the old simulation, we can immediately get the coeffs
    cat(best_WGD_sim_fn)
    WGD_sims <- readRDS(file.path(WGD_RESULTS, best_WGD_sim_fn))
...
# makes plot B
p1 <- ggplot(plot_df, aes(x = organoid, y = CnFS, fill = sim_type)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Organoid", y = "CnFS", fill = "sim_type") +
  theme_minimal()
ggsave("plot/figure_5/fig_5b.pdf", plot = p1)